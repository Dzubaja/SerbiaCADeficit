...
from dashboard.charts import (
    ...
    component_ranking_chart, yoy_change_chart,
...
# ── Navigation CSS (home page tiles) ─────────────────────────────────

_is_light = theme == "light"
_NAV_CSS = f"""
<style>
.nav-home {{
...
.nav-tile {{
    flex: 1; border-radius: 16px; padding: 36px 32px;
    ...
    {'background: #FFFFFF; border: 1px solid #E2E8F0; box-shadow: 0 2px 8px rgba(0,0,0,0.06);'
     if _is_light else
     'background: linear-gradient(145deg, #0F1629 0%, #131A2E 100%); border: 1px solid rgba(255,255,255,0.06); box-shadow: 0 4px 16px rgba(0,0,0,0.2);'}
}}
.nav-tile:hover {{
    transform: translateY(-4px);
    {'box-shadow: 0 8px 24px rgba(0,0,0,0.12);' if _is_light else 'box-shadow: 0 12px 40px rgba(0,0,0,0.4);'}
}}
.nav-tile::before {{
    content: ''; position: absolute; top: 0; left: 0; right: 0;
    height: 4px; border-radius: 16px 16px 0 0;
}}
.nav-tile-macro::before {{
    background: linear-gradient(90deg, #0891B2, #6D28D9);
}}
.nav-tile-bank::before {{
...
.nav-tile .tile-title {{
    ...
    margin-bottom: 8px;
    {'color: #1E293B;' if _is_light else 'color: #E2E8F0;'}
}}
.nav-tile .tile-desc {{
    font-size: 0.82rem; font-weight: 500; line-height: 1.5;
    {'color: #64748B;' if _is_light else 'color: #8B95A8;'}
}}
.nav-tile .tile-items {{
    font-size: 0.72rem; font-weight: 500; margin-top: 12px; line-height: 1.7;
    {'color: #94A3B8;' if _is_light else 'color: #5A6478;'}
}}
.nav-tile .tile-arrow {{
    font-size: 1.1rem; font-weight: 700; margin-top: 8px;
    {'color: #0891B2;' if _is_light else 'color: #00D4FF;'}
}}
.nav-back {{
    ...
    {'color: #0891B2; background: rgba(8,145,178,0.06);' if _is_light else 'color: #00D4FF; background: rgba(0,212,255,0.06);'}
}}
.nav-back:hover {{
    {'background: rgba(8,145,178,0.12);' if _is_light else 'background: rgba(0,212,255,0.12);'}
}}
@media (max-width: 768px) {{
    .nav-home {{ flex-direction: column; gap: 16px; }}
    .nav-tile {{ min-height: auto; padding: 24px 20px; }}
    .nav-tile .tile-title {{ font-size: 1.1rem; }}
}}
</style>
"""
...
if st.session_state["page"] == "home":
    ...
    with c1:
        st.markdown("""
        <div class="nav-tile nav-tile-macro">
            <div>
                <div class="tile-title">Current Account & FDI</div>
                <div class="tile-desc">
                    Macro external sector analysis — balance of payments,
                    trade flows, foreign direct investment, reserves, and debt
                </div>
                <div class="tile-items">
                    Current Account &nbsp;·&nbsp; FDI Deep Dive &nbsp;·&nbsp;
                    BOP Rankings &nbsp;·&nbsp; Trade & Finance &nbsp;·&nbsp;
                    External Position
                </div>
            </div>
            <div class="tile-arrow">Explore →</div>
        </div>
        """, unsafe_allow_html=True)
        if st.button("Open Current Account & FDI", key="btn_macro",
    ...
    with c2:
        st.markdown("""
        <div class="nav-tile nav-tile-bank">
            <div>
                <div class="tile-title">Banking Sector Overview</div>
                <div class="tile-desc">
                    Competitive intelligence — individual bank analysis,
                    peer benchmarking, profitability, growth, and market position
                </div>
                <div class="tile-items">
                    Sector KPIs &nbsp;·&nbsp; Bank Deep Dive &nbsp;·&nbsp;
                    Rankings &nbsp;·&nbsp; Peer Comparison &nbsp;·&nbsp;
        ...
        """, unsafe_allow_html=True)
        if st.button("Open Banking Sector", key="btn_bank",
                     use_container_width=True, type="primary"):
            st.session_state["page"] = "banking"
            st.rerun()

    # Quick stats on home
    st.markdown("<div style='margin-top: 1.5rem'></div>", unsafe_allow_html=True)
    st.markdown('<div class="section-header">Key Indicators at a Glance</div>',
                unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════
# STREAM 1: CURRENT ACCOUNT & FDI
# ══════════════════════════════════════════════════════════════════════

elif st.session_state["page"] == "macro":
    if st.button("← Home", key="back_macro"):
        st.session_state["page"] = "home"
        st.rerun()
    ...
    with tab2:
            ...
            with fk4:
                if cov_val is not None:
                    st.markdown(kpi_card(
                        ...
                        sub_text=str(fdi_year), change=cov_chg,
                    ), unsafe_allow_html=True)

        st.markdown("<div style='margin-top: 0.8rem'></div>", unsafe_allow_html=True)

        fdi_tab1, fdi_tab2, fdi_tab3 = st.tabs([
            "  By Country  ", "  By Sector  ", "  Analytics  ",
        ])

        with fdi_tab1:
            fc1, fc2 = st.columns([1, 3])
            with fc1:
                top_n = st.selectbox("Show top N countries", [5, 10, 15, 20],
                                     index=1, key="fdi_top_n")
        ...
        with fdi_tab3:
            ...
            if not fdi_analytics.empty and "Net FDI" in fdi_analytics.columns:
                ca_data_a = data["ca"]
                _c = fdi_analytics[["year", "Net FDI"]].merge(ca_data_a, on="year", suffixes=("_fdi", "_ca"))
...
# ══════════════════════════════════════════════════════════════════════
# STREAM 2: BANKING SECTOR
# ══════════════════════════════════════════════════════════════════════

elif st.session_state["page"] == "banking":
    if st.button("← Home", key="back_bank"):
        st.session_state["page"] = "home"
        st.rerun()
